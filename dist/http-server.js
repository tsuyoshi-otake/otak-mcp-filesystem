...
app.get('/stream/watch', async (req, res) => {
    ...
    try {
        const stats = await promises_1.default.stat(watchPath);
        sendEvent({
            type: 'initial',
            path: watchPath,
            exists: true,
        ...
        const watcher = promises_1.default.watch(watchPath, { recursive: true });
...
app.get('/stream/tail', async (req, res) => {
    ...
    try {
        ...
        const watcher = promises_1.default.watch(filePath);
        for await (const event of watcher) {
            if (event.eventType === 'change') {
                const newStats = await promises_1.default.stat(filePath);
